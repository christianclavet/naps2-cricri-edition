...
namespace NAPS2.Ocr
{
    ...
    public class Tesseract531Engine : TesseractBaseEngine
    {
        ...
        public Tesseract531Engine(AppConfigManager appConfigManager, ComponentManager componentManager) : base(appConfigManager)
        {
            string exeFolder = "w64";
            LanguageData = TesseractLanguageData.V400B4;
            TesseractBasePath = Path.Combine(componentManager.BasePath, "tesseract-5.31");
            TesseractExePath = Path.Combine(exeFolder, "tesseract.exe");
            PlatformSupport = PlatformSupport.ModernWindows;
            CanInstall = false;
            SupportedModes = new[] {OcrMode.Best};
